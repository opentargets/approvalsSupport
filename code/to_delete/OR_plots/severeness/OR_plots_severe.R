...
data  <- subset(data_in, has_GE != "NA" & Disease_class == "Severe")  
...
write.csv(merged_tibble, file = "./output/v3/OR/OR_v3.4_onco_2x2.csv", row.names = TRUE)
...
write.table(df, sep = ",", file = "./output/v3/OR/OR_v4_severe_only.csv", row.names = FALSE)
...
OR <- ggplot(df, aes(x = estimate, y = table_data)) +
  ...
  scale_x_log10(limits = c(0.2, 550), labels = function(x) ifelse(x > 1, round(x), x)) +
...
ggsave("./output/v3/OR/OR_v4_severe_only.png", 
        width = 4,
        height = 4)

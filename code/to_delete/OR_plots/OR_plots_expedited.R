...
data  <- subset(data_in, has_GE != "NA" & !(grepl("^S$|^S, O$", Review_type)))

# write.table(data, sep = ",", file = "./output/v3/OR/OR_v3.4_exp_test.csv", row.names = FALSE)
...
write.csv(merged_tibble, file = "./output/v3/OR/OR_v3.4_exp_2x2.csv", row.names = TRUE)
...
odds_ratio_6 <- odds_ratio_df(table_data_S)
...
write.table(df, sep = ",", file = "./output/v3/OR/OR_v3.4_exp.csv", row.names = FALSE)
...
OR <- ggplot(df, aes(x = estimate, y = table_data)) +
  ...
  scale_x_log10(limits = c(0.03, 400), labels = function(x) ifelse(x > 1, round(x), x)) +
...
ggsave("./output/v3/OR/OR_v3.4_exp_tm.png", 
        width = 4,
        height = 4)

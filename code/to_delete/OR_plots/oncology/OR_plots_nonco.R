...
list_of_matrix_to_merge <- lapply(list(
  ...
  table_data_F,
  table_data_E,
  table_data_nE), as.data.frame.matrix)
merged_tibble <- do.call(cbind, list_of_matrix_to_merge)

write.csv(merged_tibble, file = "./output/v3/OR/OR_v3.4_nonco_2x2.csv", row.names = TRUE)
...
write.table(df, sep = ",", file = "./output/v3/OR/OR_v3.4_nonco.csv", row.names = FALSE)
...
OR <- ggplot(df, aes(x = estimate, y = table_data)) +
  ...
  scale_x_log10(limits = c(0.3, 10), labels = function(x) ifelse(x > 1, round(x), x)) +
...
ggsave("./output/v3/OR/OR_v3.4_nonco_tm.png", 
        width = 4,
        height = 4)

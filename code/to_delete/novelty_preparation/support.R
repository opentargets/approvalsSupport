...
interactors_ass <- approvals %>%
    ...
    select(datasourceId, Drug_name, targetB) %>%
    sdf_distinct() %>%
    collect() %>%
    mutate(interactionAssociation = TRUE)

# write.table(interactors_ass, sep = ",", file = "./output/v3/Interactors_v3.2.csv", row.names = FALSE)
...
ass %>%
    ...
    summarise(score = suppressWarnings(max(score, na.rm = TRUE))) %>%
        ...
        left_join(approvals %>% select(Drug_name, Year, Brand_name,	Drug_name_original) %>% collect(), by = "Drug_name") %>% colnames()
...
write.table(data2plot, sep = ",", file = "./output/v3/2013-2022_approvals_GE_v3.3_out_targetB.csv", row.names = FALSE)

...
data <- read_csv("./output/v3/2013-2022_approvals_GE_v3.7_oncocheck.csv")
...
data_onco  <- subset(data, TA == "Oncology") 

data_onco_count <- data_onco %>%
    select(Drug_name_original, Year) %>%
    distinct() %>%
    group_by(Year) %>%
    summarise(n_onco = n()) %>%
...
data_groups  <- subset(data, TA == "Oncology" & !(grepl("^S$|^S, O$", data$Review_type))) 


all_Exp_count <- data_groups %>%
            select(Drug_name_original, Year) %>% 
            distinct() %>%
            group_by(Year) %>%
            ...
            left_join(data_onco_count) %>%
            mutate(Exp_onco_rate = Exp_count / n_onco * 100) %>%
            mutate(onco_rate = n_onco / n * 100) 

# ggplot(all_S_count, aes(x = as.integer(Year), y = S_rate)) +
#       geom_point() +
#       geom_smooth(method = "lm", se = TRUE) +
#       xlab("Year") + 
#       ylab("Standard, %") +
#       stat_cor(label.x = 2015, label.y = 50) +
#       stat_regline_equation(label.x = 2015, label.y = 47) +
#       scale_x_continuous(limits = c(2013, 2022)) +
#       geom_text(label = as.integer(all_S_count$S_rate), nudge_y = 2.5) +
#       theme_light()

# ggsave("./output/S_regr_v1.png", 
#         width = 6,
#         height = 3)

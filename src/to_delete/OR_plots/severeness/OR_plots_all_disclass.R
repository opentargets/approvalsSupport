...
colnames(table_data_E) <- c("", "Expedited programs")
...
write.csv(merged_tibble, file = "./output/v3/OR/OR_v5_all_Serious_2x2_3.csv", row.names = TRUE)
...
# odds_ratio_10 <- odds_ratio_df(table_data_FDA)
odds_ratio_11 <- odds_ratio_df(table_data_Serious)
...
write.table(df, sep = ",", file = "./output/v3/OR/OR_v5_all_Serious_3.csv", row.names = FALSE)
...
OR <- ggplot(df, aes(x = estimate, y = table_data)) +
  ...
  scale_x_log10(limits = c(0.5, 10), labels = function(x) ifelse(x > 1, round(x), x)) +
...
ggsave("./output/v3/OR/OR_v5_all_Serious_3.png", 
        width = 4,
        height = 1.6)

...
data  <- subset(data_in, has_GE != "NA" & grepl("O", Review_type))  
...
data$Expedited <- !(grepl("^S, O$", data$Review_type))
...
list_of_matrix_to_merge <- lapply(list(
  ...
  # table_data_S,
  table_data_F,
  table_data_E), as.data.frame.matrix)
merged_tibble <- do.call(cbind, list_of_matrix_to_merge)

write.csv(merged_tibble, file = "./output/v3/OR/OR_v3.4_orphan_2x2.csv", row.names = TRUE)
...
write.table(df, sep = ",", file = "./output/v3/OR/OR_v3.4_orphan.csv", row.names = FALSE)
...
ggsave("./output/v3/OR/OR_v3.4_orphan.png", 
        width = 4,
        height = 4)

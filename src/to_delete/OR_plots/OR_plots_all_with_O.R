...
write.csv(merged_tibble, file = "./output/v3/OR/OR_v3.4_all_O_2x2.csv", row.names = TRUE)
...
write.table(df, sep = ",", file = "./output/v3/OR/OR_v3.4_al_O.csv", row.names = FALSE)
...
OR <- ggplot(df, aes(x = estimate, y = table_data)) +
  ...
  scale_x_log10(limits = c(0.5, 130), labels = function(x) ifelse(x > 1, round(x), x)) +
...
ggsave("./output/v3/OR/OR_v3.4_all_O.png", 
        width = 7,
        height = 4)

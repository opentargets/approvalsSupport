...
my_colors <- c("#f2b7ae", "#b8ccda", "#6caad9")
...
ggplot(aes(x=Year, y=fraction, fill=fct_reorder(type, fraction))) +
  ...
  geom_text(aes(x = Year, y = label_ypos, label = count), size = 3.5, family="Helvetica", color = "#303030") +
  theme_minimal() +
  ylab("Approvals, %") +
  ...
  theme(axis.text.x = element_text(vjust = -0.6),
        ...
        text = element_text(family="Helvetica", size = 12, color = "#4D4D4D"))
...
ggsave("./output/nature_plot/GE_OR_v4.png", 
        width = 8,
        height = 3.9)

...
data  <- subset(data_in, has_GE != "NA" & TA == "Oncology")  
...
write.table(df, sep = ",", file = "./output/v3/OR/OR_v3.4_onco.csv", row.names = FALSE)
...
OR <- ggplot(df, aes(x = estimate, y = table_data)) +
  ...
  scale_x_log10(limits = c(0.01, 550), labels = function(x) ifelse(x > 1, round(x), x)) +
...
ggsave("./output/v3/OR/OR_v3.4_onco_tm.png", 
        width = 4,
        height = 4)

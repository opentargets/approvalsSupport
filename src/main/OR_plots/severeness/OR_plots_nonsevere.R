...
### PLOT 2 (OR)
data_in <- read_csv("./output/v3/2013-2022_approvals_GE_v4_severe.csv")

# data  <- subset(data_in, has_GE != "NA" & TA == "Other")  
data  <- subset(data_in, has_GE != "NA" & Disease_class == "Non Severe")  
...
data$FDA_programs <- !(grepl("^S$", data$Review_type))
...
list_of_matrix_to_merge <- lapply(list(
  ...
  table_data_E,
  table_data_nE,
  table_data_FDA), as.data.frame.matrix)
merged_tibble <- do.call(cbind, list_of_matrix_to_merge)

write.csv(merged_tibble, file = "./output/v3/OR/OR_v3.4_nonsevere_2x2.csv", row.names = TRUE)
...
odds_ratio_df_catch <- function(data) {
  tryCatch(
    {
      result <- odds_ratio_df(data)
      return(result)
    },
    error = function(e) {
      return(data.frame())
...
odds_ratio_0 <- odds_ratio_df_catch(table_data_A)
odds_ratio_1 <- odds_ratio_df_catch(table_data_P)
odds_ratio_2 <- odds_ratio_df_catch(table_data_B)
odds_ratio_3 <- odds_ratio_df_catch(table_data_O)
odds_ratio_4 <- odds_ratio_df_catch(table_data_S)
odds_ratio_5 <- odds_ratio_df_catch(table_data_F)
odds_ratio_6 <- odds_ratio_df_catch(table_data_E)
odds_ratio_7 <- odds_ratio_df_catch(table_data_nE)
odds_ratio_8 <- odds_ratio_df_catch(table_data_FDA)
...
write.table(df, sep = ",", file = "./output/v3/OR/OR_v4_nonsevere_only.csv", row.names = FALSE)
...
OR <- ggplot(df, aes(x = estimate, y = table_data)) +
  ...
  scale_x_log10(limits = c(0.05, 200), labels = function(x) ifelse(x > 1, round(x), x)) +
...
#   theme_bw() +
  theme(axis.text.x = element_text(vjust = -0.6),
        ...
        plot.margin = margin(t = 4, r = 1, b = 0, l = 0)) +
...
ggsave("./output/v3/OR/OR_v4_nonsevere_only.png", 
        width = 4,
        height = 4)

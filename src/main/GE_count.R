...
# write.table(data_humT_fraction, sep = ",", file = "./output/data_humT_fraction.csv", row.names = FALSE)
...
custom_order <- c("noH", "without_GE", "GE_noH")

data_to_plot <- data_humT_fraction %>% 
    mutate(type = fct_relevel(type, custom_order)) %>%
    arrange(type) %>%
    left_join(data_humT_count, by = c("Year", "type")) %>%
    group_by(Year) %>%
    mutate(label_ypos = 100 - (cumsum(fraction) - fraction / 2)) %>%
    ungroup() %>%
    mutate(Year = as.factor(Year), 
...
# my_colors <- c("#6caad9","#f2b7ae", "#b8ccda")



# GE <- data_to_plot %>%
# ggplot(aes(x=Year, y=fraction, fill=fct_relevel(type, custom_order))) +
#   geom_bar(stat="identity", width = 0.7) +
#   scale_y_continuous(limits = c(0,100.01), expand = c(0, 0), labels = scales::percent_format(scale = 1)) +
#   scale_fill_manual(values = my_colors,
#                     breaks = c("GE_noH", "without_GE", "noH"),
#                     labels = c("With evidence", "Without evidence", "Non-human target"),
...
# all_odds_ratios <- rbind(odds_ratio_0,
#                         odds_ratio_1,
#                         odds_ratio_2,
#                         odds_ratio_3,
#                         odds_ratio_4,
...
#   scale_x_log10(limits = c(0.2, 700), labels = function(x) ifelse(x > 1, round(x), x)) +
...
# # ggsave("./output/v3_full/OR/OR_v1.png", 
# #         width = 10,
# #         height = 5)
